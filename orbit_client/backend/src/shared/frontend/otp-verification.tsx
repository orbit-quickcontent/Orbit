...
export default function OTPVerification({ email, role, onVerified, onBack }: OTPVerificationProps) {
  ...
  const sendOtpViaApi = useCallback(async (emailAddr: string) => {
    ...
    return data.demoOtp as string | undefined;
  }, []);
  ...
  const sendOtp = useCallback(async () => {
    ...
    try {
      ...
      if (useApiFallback) {
        // Client-side mode (used in sandbox/demo environments)
        generatedOtp = sendOtpClientSide(email);
      } else {
        // Try API first, fall back to client-side on failure
        try {
          generatedOtp = await sendOtpViaApi(email);
        } catch {
          // API unavailable — switch to client-side mode permanently
          console.warn("[OTP] API unavailable, using client-side OTP generation");
          setUseApiFallback(true);
          generatedOtp = sendOtpClientSide(email);
        }
      }

      // Show the OTP code for demo (in production, this would be sent via email)
      if (generatedOtp) {
          ...
          {demoOtp && (
            ...
            >
              <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-yellow-500/10 border border-yellow-500/20">
                <span className="text-xs text-yellow-400/80">Demo OTP:</span>
                <span className="text-sm font-mono font-bold text-yellow-300 tracking-widest">
                  {demoOtp}
                </span>
              </div>
            </motion.div>
