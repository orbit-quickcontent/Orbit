...
function buildQuery(colRef, filter, sorting) {
  let q = query(colRef);
...
export const firestoreDb = {
  clientUsers: {
    ...
    findMany: async (args) => {
      ...
      const q = buildQuery(colRef, _optionalChain([args, 'optionalAccess', _ => _.where]));
      const snap = await getDocs(q);
  ...
  partnerUsers: {
    ...
    findMany: async (args) => {
      ...
      const q = buildQuery(colRef, _optionalChain([args, 'optionalAccess', _2 => _2.where]));
      const snap = await getDocs(q);
      return snap.docs.map((d) => parseDates({ id: d.id, ...(d.data() ) }));
    },
    upsert: async ({ where, update, create }) => {
  ...
  bookings: {
    findMany: async (args) => {
      ...
      const q = buildQuery(colRef, _optionalChain([args, 'optionalAccess', _3 => _3.where]), _optionalChain([args, 'optionalAccess', _4 => _4.orderBy]));
      const snap = await getDocs(q);
      return snap.docs.map((d) => parseDates({ id: d.id, ...(d.data() ) }));
    },
    findUnique: async ({ where }) => {
  ...
  partners: {
    findMany: async (args) => {
      ...
      const q = buildQuery(colRef, _optionalChain([args, 'optionalAccess', _5 => _5.where]));
      const snap = await getDocs(q);
      return snap.docs.map((d) => parseDates({ id: d.id, ...(d.data() ) }));
    },
    findUnique: async ({ where }) => {
  ...
  workDispatches: {
    findMany: async (args) => {
      ...
      const q = buildQuery(colRef, _optionalChain([args, 'optionalAccess', _6 => _6.where]));
      const snap = await getDocs(q);
      return snap.docs.map((d) => parseDates({ id: d.id, ...(d.data() ) }));
    },
    findFirst: async ({ where }) => {
  ...
  transactions: {
    findMany: async (args) => {
      ...
      const q = buildQuery(colRef, _optionalChain([args, 'optionalAccess', _7 => _7.where]), _optionalChain([args, 'optionalAccess', _8 => _8.orderBy]));
      const snap = await getDocs(q);
      return snap.docs.map((d) => parseDates({ id: d.id, ...(d.data() ) }));
    },
    create: async ({ data }) => {
  ...
  emailOtps: {
    ...
    create: async ({ data }) => {
      ...
      const payload = {
        ...
        verified: _nullishCoalesce(data.verified, () => ( false)),
        attempts: _nullishCoalesce(data.attempts, () => ( 0)),
        used: _nullishCoalesce(data.used, () => ( false)),
      };
