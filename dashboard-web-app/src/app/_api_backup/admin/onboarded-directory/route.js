...
export async function GET() {
  try {
    ...
    const bookingsByPartner = new Map();
    ...
    const partnerDirectory = await Promise.all(
      partners.map(async (p) => {
        ...
        return {
          id: p.id,
          userId: p.userId,
          name: _optionalChain([u, 'optionalAccess', _ => _.name]) || "N/A",
          email: _optionalChain([u, 'optionalAccess', _2 => _2.email]) || "N/A",
          phone: _optionalChain([u, 'optionalAccess', _3 => _3.phone]) || "N/A",
          avatar: _optionalChain([u, 'optionalAccess', _4 => _4.avatar]),
          location: p.location,
    ...
    const clientDirectory = await Promise.all(
      clientUsers.map(async (c) => {
        ...
        const spent = completed.reduce((sum, b) => {
          ...
          return sum + (_optionalChain([pkg, 'optionalAccess', _5 => _5.price]) || 0);
        }, 0);
    ...
    const bookingsList = bookings.map((b) => {
      ...
      return {
        id: b.id,
        packageName: _optionalChain([pkg, 'optionalAccess', _6 => _6.name]) || "Personalized",
        packagePrice: _optionalChain([pkg, 'optionalAccess', _7 => _7.price]) || 1999,
        clientName: _optionalChain([u, 'optionalAccess', _8 => _8.name]) || "N/A",
        clientEmail: _optionalChain([u, 'optionalAccess', _9 => _9.email]) || "N/A",
        status: b.status,
